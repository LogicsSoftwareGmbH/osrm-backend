...
{
    std::size_t appended = 0;         // no duplicate existed, edge inserted
    std::size_t updated_in_place = 0; // duplicate existed, smaller weight written
    std::size_t dropped = 0;          // duplicate existed, candidate not better
};
 ...
 * a middle node, so they cannot be updated in place.
 *
 * @param graph
 ...
 *
 * A shortcut for a node pair may already exist: the bounded witness search of a
 * later round can regenerate a shortcut instead of finding the copy an earlier
 * round inserted. Keeping the smaller weight in the existing edge instead of
                 ...
                 InsertEdgeStats &stats)
{
    ...
    {
        const EdgeID duplicate = FindDuplicateShortcut(graph, edge);
        if (duplicate != SPECIAL_EDGEID)
        {
            auto &data = graph.GetEdgeData(duplicate);
            if (edge.data.weight < data.weight)
            {
                data = edge.data;
                ++stats.updated_in_place;
            }
            else
            {
                ++stats.dropped;
                                ...
                                double core_factor)
{
    ...
    util::Log() << "shortcut duplicates: " << insert_stats.updated_in_place
                << " updated in place, " << insert_stats.dropped << " dropped, "
                << insert_stats.appended << " edges appended";
